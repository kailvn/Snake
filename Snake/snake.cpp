...
void Snake::AI_find_path(Food &f)
{
	...
	for (unsigned int i = 0; i < body.size(); i++)
	{
		map[(body[i].X-3)/2][body[i].Y-2] = true; //蛇的身体也是障碍
	}
	COORD begin_point, end_point;

	begin_point = tran_c(body.back());//转为小地图
	end_point = tran_c(fpoint);//转为小地图

	AISnake.GetPath(begin_point, end_point);//小地图

	
	if (!AISnake.paths_queue.empty())
	{
		head = retran_c(AISnake.paths_queue.front());//转为大地图
